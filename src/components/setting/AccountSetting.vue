...
import { changePassword, login } from '@/api/user'
...
function validOldPass(_rule: any, value: any, callback: any) {
  if (value === '') {
...
function validNewPass(_rule: any, value: any, callback: any) {
  // 校验确认密码
...
function validConfirmNewPass(_rule: any, value: any, callback: any) {
  if (value === '') {
...
async function loginFunc(): Promise<any> {
  ...
  const userName = store.userInfo.user_name ?? localUserInfo.user_name
...
<template>
  <div>
    <el-form ref="formRef" :model="form" status-icon :rules="rules" label-width="100px" label-position="left">
      ...
      <el-form-item>
        <el-button plain type="primary" @click="submit(formRef)">
          保存
        </el-button>
      </el-form-item>
    </el-form>
